...
function get_category(id,next,select_id){
    ...
    //如果为顶级分类
    if(id==0){
        $("#"+next).html(html);//二级分类为 请选择商品分类
        $("#parentId").val(id);//一级分类value==0
        $("#level").val(1);//一级分类等级为1
        return;
    }
    //如果不为顶级分类
    $("#parentId").val(id);//一级分类为下拉框的内容
    $("#level").val(2);//一级分类等级为2
    var url = getProjectName() + "/product/category/getByParentId";
    $.ajax({
        url:url,
        type:"post",
        data:{
 ...
 * 获取多级联动的商品分类
 */
function get_category_goods(id, next, hidden) {
    var html = "<option value='0'>请选择商品分类</option>";
    // 如果顶级分类为0
    if (0 == id) {
        $("#" + next).html(html);
        var next_id = next.split("_");
        var num = parseInt(next_id[2]) + 1;
        next_id = next_id[0] + "_" + next_id[1] + "_" + num;
        $("#" + next).html(html);
        $("#" + next_id).html(html);
        $("#" + hidden).val(id);
        return;
...
/**
 * 商品设置catId和extendCatId
 */
function setCatIdAndExtendCarId(val, id) {
    // 如果是请选择
    if (0 == val) {
        $("#" + id).val(val);
        return;
